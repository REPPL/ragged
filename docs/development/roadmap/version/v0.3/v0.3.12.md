...
**Why Required:** v0.3.0 launches REST API server supporting multiple users. v0.2.10 provides session isolation to prevent cross-user data leakage, and v0.2.11 provides encryption and privacy controls for API requests.
...
**Priority:** High
**Dependencies:** v0.3.0 (RAGAS), v0.3.0 (query decomposition)
...
Did you mean?
  → "What are the main findings?"  (spelling correction)

Suggested refinements:
  → "What are the main findings of this research?"  (more specific)
  → "Summarise the key findings"  (clearer intent)
...
📄 Retrieved 5 chunks (0.94 avg confidence)
...
Reason: Vague keyword → specific question

📄 Retrieved 5 chunks (0.89 avg confidence)
...
- Spelling correction
- Query refinement (vague → specific)
...
Available themes:
  ...
  • colourblind-safe - Colourblind-friendly
  • monochrome       - No colours (accessible)
  • custom           - User-defined (see config.yaml)
```
...
async def query_documents(
    request: QueryRequest,
    api_key: str = Depends(get_api_key)  # ✅ Authentication required
):
...
⚠️  This will invalidate the old key. Continue? (y/n): y
✓ New key: ak_xxxxxxxxxxx
✓ Old key invalidated
```
...
**Testing:**
- [ ] Test session isolation (Client A ≠ Client B cache)
...
│   └── app: FastAPI
├── routes/
│   ├── documents.py            # Document endpoints (200 lines)
│   ├── query.py                # Query endpoints (150 lines)
│   ├── metrics.py              # Metrics endpoints (100 lines)
│   └── health.py               # Health check (50 lines)
├── models.py                   # Pydantic models (200 lines)
├── auth.py                     # Authentication (150 lines)
└── middleware.py               # CORS, rate limiting (100 lines)

src/generation/
└── suggestions.py              # Smart suggestions (250 lines)
    └── class QuerySuggester

src/cli/
└── themes.py                   # Theme management (200 lines)
    └── class ThemeManager
...
### Pre-Implementation
- [ ] Create branch: `git checkout -b feature/v0.3.12-polish-integration`
- [ ] Use ux-architect agent for API and accessibility design
...
- ✅ API examples provided (Python, JavaScript)
- ✅ Smart suggestions improve query quality
- ✅ All themes work correctly and meet WCAG 2.1 AA
- ✅ Screen reader compatible
- ✅ All v0.3.x features integrated and tested
- ✅ Complete documentation for entire v0.3.x
...
1. **v0.3.0** - Foundation & Metrics (RAGAS baseline)
2. **v0.3.0** - Configuration Transparency (personas, presets)
3. **v0.3.0** - Advanced Query Processing (query decomposition, HyDE, reranking, compression)
4. **v0.3.0** - Intelligent Chunking (semantic, hierarchical)
5. **v0.3.0** - Modern Document Processing (Docling, 30× performance)
6. **v0.3.0** - Messy Document Intelligence (automated PDF correction)
7. **v0.3.0** - VectorStore Abstraction (multi-database support)
8. **v0.3.0** - Production Data & Generation (version tracking, metadata filtering, chain-of-thought)
9. **v0.3.0** - Developer Experience I (REPL, debug mode)
10. **v0.3.0** - Performance & Quality Tools (profiling, metrics dashboard)
11. **v0.3.0** - Automation & Templates (Jinja2 templates, testing utilities)
12. **v0.3.0** - Production Operations (watch mode, scheduled operations)
13. **v0.3.0** - Polish & Integration (REST API, smart suggestions, accessibility)
