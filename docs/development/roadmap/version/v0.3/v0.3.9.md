# v0.3.9 - Performance & Quality Tools
...
**Why Required:** v0.3.0 stores query metrics in SQLite database. v0.2.11 provides query hashing, encryption, and TTL policies to protect user privacy.
...
**Priority:** High
**Dependencies:** v0.3.0 (debug mode for integration)
...
⏱️ Performance Profile: Query Pipeline

Pipeline Breakdown:
┌──────────────────────┬──────────┬─────────┬─────────┐
│ Stage                │ Duration │ %Total  │ Memory  │
├──────────────────────┼──────────┼─────────┼─────────┤
│ Query Preprocessing  │ 2ms      │ 0.1%    │ 0.5MB   │
│ Query Embedding      │ 45ms     │ 3.3%    │ 12MB    │
│ Vector Retrieval     │ 23ms     │ 1.7%    │ 8MB     │
│ Reranking            │ 67ms     │ 4.9%    │ 24MB    │
│ Context Building     │ 3ms      │ 0.2%    │ 2MB     │
│ Generation (LLM)     │ 1,234ms  │ 89.8%   │ 156MB   │
└──────────────────────┴──────────┴─────────┴─────────┘

Total: 1,374ms (Memory peak: 202MB)

🔍 Bottleneck Analysis:
  ⚠️  Generation (LLM) taking 89.8% of total time
      → Consider using faster model or caching
  ✓   Vector retrieval optimal (23ms for 5 chunks)
  ✓   Reranking acceptable (67ms)
...
⏱️ Performance Profile: Document Ingestion

Ingestion Breakdown:
┌──────────────────────┬──────────┬─────────┬─────────┐
│ Stage                │ Duration │ %Total  │ Memory  │
├──────────────────────┼──────────┼─────────┼─────────┤
│ PDF Loading          │ 234ms    │ 2.1%    │ 45MB    │
│ OCR Processing       │ 8,456ms  │ 75.2%   │ 512MB   │
│ Markdown Conversion  │ 123ms    │ 1.1%    │ 28MB    │
│ Chunking             │ 456ms    │ 4.1%    │ 67MB    │
│ Embedding Generation │ 1,876ms  │ 16.7%   │ 234MB   │
│ Vector Storage       │ 89ms     │ 0.8%    │ 12MB    │
└──────────────────────┴──────────┴─────────┴─────────┘

Total: 11,234ms (Memory peak: 898MB)

🔍 Bottleneck Analysis:
  ⚠️  OCR Processing taking 75.2% of total time
      → Document quality: Good (using Docling)
      → Consider GPU acceleration for batch processing
  ⚠️  Embedding generation taking 16.7%
      → Batch size: 32 (optimal)
  ✓   Vector storage optimal (89ms)
```
...
📊 ragged Quality Metrics Dashboard
...
├─────────────────────────┼────────┼────────┼────────┼────────┤
│ Context Precision       │ 0.87   │ 0.89   │ 0.95   │ 0.98   │
│ Context Recall          │ 0.82   │ 0.84   │ 0.91   │ 0.94   │
│ Faithfulness            │ 0.91   │ 0.93   │ 0.97   │ 0.99   │
│ Answer Relevancy        │ 0.85   │ 0.87   │ 0.93   │ 0.96   │
└─────────────────────────┴────────┴────────┴────────┴────────┘

Overall RAGAS: 0.86 (Target: > 0.80) ✅
...
Quality Trends (last 7 days):
  📈 Context Precision: +0.03 (0.84 → 0.87)
  📈 Faithfulness: +0.02 (0.89 → 0.91)
  📉 Answer Relevancy: -0.01 (0.86 → 0.85)
...
├─────────────────────────┼──────────┼──────────┼────────┤
│ Context Precision       │ 0.87     │ 0.89     │ +0.02  │
│ Context Recall          │ 0.82     │ 0.85     │ +0.03  │
│ Faithfulness            │ 0.91     │ 0.90     │ -0.01  │
│ Answer Relevancy        │ 0.85     │ 0.88     │ +0.03  │
│ Overall RAGAS           │ 0.86     │ 0.88     │ +0.02  │
│ Avg Query Time          │ 1,374ms  │ 1,567ms  │ +193ms │
└─────────────────────────┴──────────┴──────────┴────────┘
...
class MetricsDatabase:
    def record_query(
        ...
        self.conn.execute("""
            INSERT INTO queries (
                session_id,
                query_hash,        -- ✅ HASHED (v0.2.11)
                -- NOT: query_text  ❌ Would be PII violation
                duration_ms,
                chunks_retrieved,
                avg_confidence,
                context_precision,
...
CREATE TABLE queries (
    ...
    query_hash TEXT NOT NULL,           -- ✅ HASHED query (v0.2.11)
    duration_ms REAL,
    ...
    expires_at DATETIME NOT NULL,       -- ✅ TTL expiration (v0.2.11)
...
Users can customise metrics retention:
...
✓ Metrics TTL set to 30 days

# Clear all metrics
$ ragged cleanup metrics --all
⚠️  Delete all 1,247 metrics records? (y/n): y
✓ Deleted 1,247 records
✓ Database vacuumed (reclaimed 2.3 MB)
```
...
Your Metrics Data:
  ...
  Privacy Status:
    ✓ Queries stored as hashes (not plaintext)
    ✓ TTL enabled (90 days)
    ✓ Session isolation enabled
    ✓ Auto-cleanup active
```
...
def record_session_metrics(
    ...
    db.record_query(
        ...
        metrics=metrics,
        session_id=current_session.session_id  # ✅ Session-scoped
    )
```
...
```
src/cli/commands/
├── profile.py                  # Performance profiling (250 lines)
│   └── class Profiler
└── metrics.py                  # Quality metrics (350 lines)
    └── class MetricsCollector

src/monitoring/
├── profiler.py                 # Core profiling (200 lines)
│   └── class PerformanceProfiler
├── metrics_storage.py          # Metrics database (150 lines)
│   └── class MetricsDB
└── dashboard.py                # Dashboard rendering (200 lines)
...
- [ ] Timing measurements accurate (±5%)
- [ ] Memory measurements accurate (±10%)
...
### Pre-Implementation
- [ ] Create branch: `git checkout -b feature/v0.3.9-performance-quality-tools`
...
- ✅ Profiling captures all pipeline stages accurately
- ✅ Bottleneck detection identifies performance issues
- ✅ Metrics dashboard shows current RAGAS scores
- ✅ Historical trends visualised clearly
- ✅ Configuration comparison enables A/B testing
- ✅ Profiling overhead <5%
...
- [v0.3.0 - Developer Experience I](./v0.3.0.md) - Debug mode integration
